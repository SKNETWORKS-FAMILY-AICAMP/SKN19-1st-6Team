...
st.title("🌱 친환경차를 몰고계신가요? 몰고다닐계획이신가요? 관련된 카드 혜택을 확인해보세요😍")
df = get_data('card')
    ...
    df.groupby(["card_company_name", "card_name"], as_index=False)
      .agg({
          ...
          **{col: any_y for col in benefit_cols.values()},  # 혜택 OR 집계
          "card_detail": lambda s: " · ".join(sorted(set(map(str, s))))  # 대표 상세혜택(중복 제거 후 합치기)
      })
...
fig = px.pie(card_counts, values="card_count", names="card_company_name", title="친환경 카드상품이 많은 카드사")
st.plotly_chart(fig, use_container_width=True)

# ID: 시각화2 - 필터 영향 없음 (혜택별 카드 카운트 중복 제거 완료)
benefit_counts = {name: (df_unique[col] == "Y").sum() for name, col in benefit_cols.items()}
...
for i, row in filtered_df.iloc[start_idx:end_idx].iterrows():
    with st.expander(f"{row['card_company_name']} - {row['card_name']}", expanded=(i == start_idx)):
